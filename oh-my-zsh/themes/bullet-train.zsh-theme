...
if [ ! -n "${BULLETTRAIN_CONTEXT_BG+1}" ]; then
  BULLETTRAIN_CONTEXT_BG=236
fi
if [ ! -n "${BULLETTRAIN_CONTEXT_FG+1}" ]; then
  BULLETTRAIN_CONTEXT_FG=25
fi
...
prompt_time() {
  if [[ $BULLETTRAIN_TIME_12HR == true ]]; then
    ...
    prompt_segment $BULLETTRAIN_TIME_BG $BULLETTRAIN_TIME_FG %D{"%D %I:%M%p"}
  fi
}
...
prompt_chars() {
  ...
  if [[ $BULLETTRAIN_PROMPT_ROOT == true ]]; then
    bt_prompt_chars="%(!.%F{red}# .%F{25}${bt_prompt_chars}%f)"
  fi
